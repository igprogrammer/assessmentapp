...
class PaymentFee extends Model
{
    ...
    public static function getAssessmentItems($paymentId){
        return DB::connection('sqlsrv')->table('payments as p')
            ...
            ->select('pf.id','payment_id','p.temp_payment_id','fee_amount','p.date_of_payment',
                'pf.account_code','p.month','p.year','fname','fyear2','fyear','fee_item_id','item_name')
            ->where(['payment_id'=>$paymentId])->orderBy('fyear','ASC')->paginate(30);
    }
}
